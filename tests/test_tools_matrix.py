...
def test_an_unset_searxng_url_is_temporary_rather_than_refused() -> None:
    ...
    listed = rows_of(answer(settings_for(ledgers, tool_execution_route=True), WEB_PROFILE))
...
def test_an_unmounted_executor_leaves_the_web_rows_temporarily_unavailable() -> None:
    ...
    listed = rows_of(answer(settings_for(tool_execution_route=False), WEB_PROFILE))
    for tool_id in ("web_search", "read_url", "wikipedia_profile"):
        assert listed[tool_id]["state"] == str(matrix.ToolState.TEMPORARILY_UNAVAILABLE)
        assert "no tool executor" in listed[tool_id]["reason"]


def test_a_mounted_executor_names_the_provider_as_the_helper() -> None:
    listed = rows_of(answer(settings_for(tool_execution_route=True), WEB_PROFILE))
    for tool_id in ("web_search", "read_url", "wikipedia_profile"):
        assert listed[tool_id]["state"] == str(matrix.ToolState.AVAILABLE_THROUGH_HELPER)
...
def test_wikipedia_installs_nowhere_outside_the_searxng_provider() -> None:
    ...
    listed = rows_of(answer(settings_for(ledgers, tool_execution_route=True), WEB_PROFILE))
    assert listed["web_search"]["state"] == str(matrix.ToolState.AVAILABLE_THROUGH_HELPER)
...
def test_the_offered_set_reads_the_two_executing_states() -> None:
    listed = body_of(answer(settings_for(tool_execution_route=True), WEB_PROFILE))["tools"]
    assert isinstance(listed, list)
