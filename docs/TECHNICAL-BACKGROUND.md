# Technical background and historical measurements

This retained background describes earlier measurement and deployment states.
Use the [README](../README.md) and [user guide](USER-GUIDE.md) for operation.
Tool availability, listener settings, and the active deployment must be read
from the running appliance; older statements below are historical.
