...
if [ "$#" -gt 1 ]; then
    ...
    printf 'QWEN_IMAGE_PROFILES_JSON names the validated image parameters, default $HOME/qwen-webui-state/image-parameters.json\n' >&2
    exit 2
fi
...
image_profiles_json=${QWEN_IMAGE_PROFILES_JSON:-"$state_directory/image-parameters.json"}
...
if [ ! -r "$image_profiles_json" ]; then
    ...
    printf 'remote/admit-image-router.sh writes image-parameters.json into its output directory; copy the admitted file to that path or name it\n' >&2
    exit 1
fi
...
printf 'lan_launch port=%s address=%s address_source=%s open=%s signing_key=%s provider=%s\n' \
    "$server_port" "$lan_address" "$lan_address_source" "$lan_open" \
    "$signing_key_state" "$web_provider"
