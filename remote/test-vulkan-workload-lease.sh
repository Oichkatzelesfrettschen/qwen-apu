...
# radv-low-priority-env.sh scrubs the ambient environment before the exec, so a
# name it unsets would never reach the server.
if grep -q '^unset QWEN_VULKAN_WORKLOAD_LOCK$' \
    "$script_directory/radv-low-priority-env.sh"; then
    fail 'radv-low-priority-env.sh scrubs the workload lock variable'
fi
