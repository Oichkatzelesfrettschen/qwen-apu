...
# script claims its own, so the inner call is given the campaign directory and
# a distinct inner arm name and this script's own arm directory carries a
# symlink to what it wrote, keeping every retained file this script's own
# instrumentation adds and every field run-power-envelope-arm.sh already
# retains under one arm directory a summarizer can read whole.
