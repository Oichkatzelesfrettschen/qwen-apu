...
# The primary universal candidate. Qwen publishes no GGUF for this checkpoint,
# so the rung comes from a community conversion and passes the structural gate
# before it is compared: chat template hash, tokenizer identity, and vocabulary
# size against the source repository. A conversion that embeds vendor sampling
# metadata into the file exists and is deliberately not the pin, because a
# checkpoint carrying different generation defaults answers differently for a
# reason the comparison is not measuring.
...
artifact_name=Qwen3.5-2B-Q4_K_M.gguf
...
source_url=https://huggingface.co/$source_repository/resolve/$source_revision/Qwen_Qwen3.5-2B-Q4_K_M.gguf
expected_bytes=1396198496
expected_sha256=57a1085840f497d764a7fc5d346922dbde961efb54cc792ea81d694fd846a1d8
