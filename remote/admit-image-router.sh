...
if command -v chromium >/dev/null 2>&1; then
    ...
    if python3 "$script_directory/web-mcp/drive-fallback-page.py" --lane image \
        ...
        # The page generates the seed the model omitted and shows it before the
        # approval, so randomness is fixed ahead of the grant rather than after.
        dialog_seed=$(jq -r '.dialog.args.seed // empty' "$browser_report")
        case $dialog_seed in
            *'generated by this page'*)
                record browser_dialog_names_generated_seed accepted "$dialog_seed"
                ;;
            *)
                record browser_dialog_names_generated_seed refused "seed=${dialog_seed:-absent}"
                ;;
        esac
