...
if [ "$#" -ne 2 ] && [ "$#" -ne 3 ]; then
    ...
    printf '        context_target cache_type_k cache_type_v projector decode_tok_s\n' >&2
    printf '        prefill_tok_s quality\n' >&2
...
awk -F'\t' -v kind="$selector_kind" -v selector="$selector" -v field="$field" '
    ...
    NF < 13 { next }
    {
        ...
        split("id role model_file fetch_script context_default context_ceiling " \
              "context_target cache_type_k cache_type_v projector decode_tok_s " \
              "prefill_tok_s quality", names, " ")
        if (field == "") {
            for (i = 1; i <= 13; i++) { printf "%s=%s\n", names[i], $i }
        } else {
            for (i = 1; i <= 13; i++) {
