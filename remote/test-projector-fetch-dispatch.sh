...
cat >"$fixture_remote/model-registry.sh" <<'SCRIPT'
#!/bin/sh
set -eu
if [ "$#" -eq 3 ] && [ "$1" = path ] &&
...
cat >"$fixture_remote/select-projector.sh" <<'SCRIPT'
#!/bin/sh
set -eu
...
cat >"$fixture_remote/download-matched-mmproj.sh" <<'SCRIPT'
#!/bin/sh
set -eu
...
cat >"$fixture_remote/download-qwen35-4b-mmproj.sh" <<'SCRIPT'
#!/bin/sh
set -eu
: >"${QWEN_TEST_WRONG_DOWNLOADER:?}"
SCRIPT

cat >"$fixture_remote/qwen-webui-control.sh" <<'SCRIPT'
#!/bin/sh
set -eu
...
cat >"$fixture_remote/qwen-teardown.sh" <<'SCRIPT'
#!/bin/sh
exit 0
SCRIPT
...
PATH="$fixture_bin:$PATH" \
    "$fixture_remote/qwen-launch.sh" >"$temporary_directory/launch.stdout"
