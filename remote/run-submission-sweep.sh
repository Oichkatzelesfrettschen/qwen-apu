...
for configuration in $configurations; do
    ...
    if ! ssh -o BatchMode=yes "$ssh_target" \
        "cd $remote_root && env $environment QWEN_SWEEP_DIRECT=1 sh remote/qwen-launch.sh custom" \
        >"$output_directory/$label.launch" 2>&1; then
        printf 'launch failed label=%s\n' "$label" >&2
        sed -n '1,20p' "$output_directory/$label.launch" >&2
        ssh -o BatchMode=yes "$ssh_target" "cd $remote_root && sh remote/qwen-teardown.sh" >/dev/null 2>&1 || true
        continue
    fi
    ...
    ssh -o BatchMode=yes "$ssh_target" \
        "cd $remote_root && sh remote/summarize-probe.sh" \
        >"$output_directory/$label.probe" 2>&1 || true

    ssh -o BatchMode=yes "$ssh_target" \
        "cd $remote_root && sh remote/qwen-teardown.sh" \
