...
#
# `--runtime-mode router-child` measures the same arm through the router, which
# is the mode build-web-presets.sh requires of a review-only vision section: it
# joins the image row's review_model against remote/validated-tuples.tsv on
# `runtime_mode=router-child` with `projector_state=loaded` at the registry
# tuple, and a standalone row leaves that section ungenerated. Router mode moves
# the tuple off the server's own argv and into one preset section, because
# common_preset::merge overwrites each section key with the router argv's value
# of the same name. The section is generated by build-router-presets.sh over a
# one-row registry copy rather than written here, so the served geometry is the
# generator's output the appliance launch also reads.

usage() {
    printf 'usage: %s [--runtime-mode standalone|router-child] MODEL_ID OUTPUT_DIRECTORY\n' \
        "$0" >&2
...
runtime_mode=${QWEN_PROBE_RUNTIME_MODE:-standalone}
while [ "$#" -gt 0 ]; do
    case $1 in
        --runtime-mode)
            [ "$#" -ge 2 ] || usage
...
# The two modes are the two the ledger's runtime_mode column admits, so a third
# spelling reaches neither an emitted row nor a served section.
case $runtime_mode in
    standalone | router-child) ;;
    *)
        printf 'runtime mode must be standalone or router-child: %s\n' \
            "$runtime_mode" >&2
        exit 2
        ;;
esac
...
# refused rather than read as if version 1 described an unmarked format. Version
# 3 adds runtime_mode, which binds an output directory to one served shape: the
# router arm and the standalone arm differ in where the tuple comes from, so one
# directory holding both would carry two measurements under one arm label.
ledger_version=3
...
preset_generator=${QWEN_ROUTER_PRESET_GENERATOR:-$script_directory/build-router-presets.sh}
# A router-child arm carries its own tuple_id, since remote/validated-tuples.tsv
# keys on that column and the two modes measure the same geometry through two
# serving paths. Every other field of the emitted row states the tuple itself
# and stays equal to the standalone row's.
tuple_id_suffix=''
[ "$runtime_mode" = standalone ] || tuple_id_suffix=-router
# The router resolves each request to a section by the body's own model key, and
# models_autoload defaults to true, so the first request of a router arm loads
# the child and the arm's request budget covers that load.
request_model_key=''
[ "$runtime_mode" = standalone ] || request_model_key=$model_id
...
if [ "$runtime_mode" = router-child ] && [ ! -x "$preset_generator" ]; then
    printf 'router preset generator is not executable: %s\n' \
        "$preset_generator" >&2
    exit 2
fi
...
summary_header='arm	model_id	depth	batch	ubatch	cache_k	cache_v	flash_attn	projector_state	status	prompt_n	prefill_s	decode_tok_s	ring_resets	gpu_faults	vram_peak_mib	gtt_peak_mib	control_status	control_answer	health	hazard_class	server_log	runtime_mode'
...
metadata_header='ledger_version	model_id	runtime_mode	model_sha256	model_bytes	projector_sha256	projector_bytes	cache_k	cache_v	flash_attn	batch	ubatch	decode_tokens	fill_margin	control_fixture	control_answer	control_prompt_sha256'
...
metadata_row="$ledger_version	$model_id	$runtime_mode	$model_sha256	$model_bytes	$projector_sha256	$projector_bytes	$cache_type_k	$cache_type_v	$flash_attention	$batch_size	$ubatch_size	$decode_tokens	$fill_margin_tokens	$control_fixture	$control_answer	$control_prompt_sha256"
if [ -s "$metadata" ]; then
...
identity_argv=$(printf '%s ' "$0" --runtime-mode "$runtime_mode" "$@" |
    tr '\t\n' '  ')
invocation_identity_sha256=$(printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "$amdgpu_module_version" low-serialized "$runtime_mode" | sha256sum)
...
# server-models.cpp spawns each child as its own llama-server process, so the
# router's exit is what the harness waits on and the child's exit is what frees
# the device. A survivor holds the carve-out the next depth allocates from and
# would also trip this harness's own startup refusal, so an arm that leaves one
# reports it rather than measuring the next depth beside it.
wait_for_device_release() {
    release_waited=0
    while [ "$release_waited" -lt "$arm_kill_after_s" ]; do
...
# The preset a router arm serves from, generated by build-router-presets.sh over
# a one-row copy of the registry. The copy names the arm's depth as
# context_default, since the generator writes LLAMA_ARG_CTX_SIZE from that field
# and a ladder measures one depth per arm. The staged model root holds symlinks
# to the weights and to the projector this run resolved, so select-projector.sh
# inside the generator answers with the same file the standalone launch attaches
# and a QWEN_MMPROJ override reaches the section.
#
# The ctx-checkpoint and draft-pair ledgers are staged empty because
# model-registry.sh validates every row of each against the registry it is given
# and a one-row registry holds no other model. An empty checkpoint ledger writes
# LLAMA_ARG_CTX_CHECKPOINTS = 0, which is the count the standalone launch passes
# on its own argv, so the two modes prefill through the same boundary rule.
# The quarantine authority stays the tree's own: a row excluded there leaves the
# preset with no section and the arm refuses rather than measuring a tuple the
# appliance withholds.
build_router_preset() {
    preset_depth=$1
    preset_staging=$2
    ...
    preset_model_root=$preset_staging/model-root
    preset_model_directory=$preset_model_root/$(dirname -- "$model_file")
    staged_model_path=$preset_model_root/$model_file
    staged_projector_path=$preset_model_directory/$(basename -- "$projector_path")
    mkdir -p "$preset_model_directory"
    ln -sfn "$model_path" "$staged_model_path"
    ln -sfn "$projector_path" "$staged_projector_path"

    registry_row=$("$registry_reader" id "$model_id")
    preset_registry=$preset_staging/models.tsv
    {
        printf '# Generated by remote/probe-depth-projector.sh for one router arm.\n'
        printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
            "$model_id" "$(registry_row_field role)" "$model_file" \
            "$(registry_row_field fetch_script)" "$preset_depth" \
            "$preset_depth" "$(registry_row_field context_target)" \
            "$cache_type_k" "$cache_type_v" "$flash_attention" \
            "$projector_requirement" \
            "$(registry_row_field projector_fetch_script)" \
            "$(registry_row_field decode_tok_s)" \
            "$(registry_row_field prefill_tok_s)" \
            "$(registry_row_field quality)" "$(registry_row_field tier)" \
            "$batch_size" "$ubatch_size" \
            "$(registry_row_field validated_filled_depth)" \
            "$(registry_row_field validation_evidence)" \
            "$(registry_row_field raw_tool_selection)" \
            "$(registry_row_field guarded_tool_execution)" \
            "$(registry_row_field q4k_variant)"
    } >"$preset_registry"
    printf '# model_id\tctx_checkpoints\tevidence\n' \
        >"$preset_staging/ctx-checkpoints.tsv"
    printf '# pair_id\ttarget_model_id\tdraft_model_id\ttier\n' \
        >"$preset_staging/draft-pairs.tsv"

    if ! env QWEN_MODEL_REGISTRY="$preset_registry" \
        QWEN_MODEL_ROOT="$preset_model_root" \
        QWEN_CTX_CHECKPOINT_LEDGER="$preset_staging/ctx-checkpoints.tsv" \
        QWEN_DRAFT_PAIRS="$preset_staging/draft-pairs.tsv" \
        QWEN_ROUTER_INCLUDE_QUARANTINE=0 \
        "$preset_generator" "$preset_path" >"$preset_log" 2>&1; then
        printf 'router preset generation failed for %s at depth %s: %s\n' \
            "$model_id" "$preset_depth" "$preset_log" >&2
        exit 2
    fi

    # The section is the whole served tuple, so it is read back rather than
    # trusted: common_preset::merge overwrites a section key with the router
    # argv's value of the same name, and the router argv this harness builds
    # states none of these six keys precisely so the section decides them.
    preset_complaint=$(awk -v id="$model_id" -v depth="$preset_depth" \
        -v batch="$batch_size" -v ubatch="$ubatch_size" \
        -v cache_k="$cache_type_k" -v cache_v="$cache_type_v" \
        -v flash="$flash_attention" -v model="$staged_model_path" \
        -v projector="$staged_projector_path" '
        /^[[:space:]]*#/ || /^[[:space:]]*$/ { next }
        /^\[/ {
            section_count++
        ...
        END {
            if (section_count != 1) {
                printf "section_count=%d", section_count
                exit
            }
            if (section_name != id) {
            ...
            expected["LLAMA_ARG_MODEL"] = model
            expected["LLAMA_ARG_MMPROJ"] = projector
            expected["LLAMA_ARG_CTX_SIZE"] = depth
            expected["LLAMA_ARG_BATCH"] = batch
            expected["LLAMA_ARG_UBATCH"] = ubatch
            expected["LLAMA_ARG_CACHE_TYPE_K"] = cache_k
            expected["LLAMA_ARG_CACHE_TYPE_V"] = cache_v
            expected["LLAMA_ARG_FLASH_ATTN"] = flash
            expected["LLAMA_ARG_CTX_CHECKPOINTS"] = 0
            for (key in expected) {
                if (!(key in seen)) {
                ...
                if (seen[key] != expected[key]) {
                    printf "%s=%s", key, seen[key]
                    exit
                }
            }
        }' "$preset_path")
    if [ -n "$preset_complaint" ]; then
        printf 'router preset section does not serve the registry tuple: %s (%s)\n' \
            "$preset_complaint" "$preset_path" >&2
        exit 2
    fi
}

registry_row_field() {
    printf '%s\n' "$registry_row" | sed -n "s/^$1=//p"
}
...
#
# A router launch states the same placement, slot count, thread count, and cache
# policy and leaves depth, cache triple, submission geometry, checkpoint count,
# and the projector to the one preset section, since common_preset::merge
# overwrites every child's key of the same name with the router argv's value.
# --models-max 1 holds one child resident, which is the shape a review section
# serves under.
start_server() {
    start_depth=$1
    start_log=$2
    if [ "$runtime_mode" = router-child ]; then
        env LLAMA_NO_CPU_FALLBACK=1 DISPLAY= WAYLAND_DISPLAY= \
            ...
            --models-preset "$arm_router_preset" \
            --models-max 1 \
            --host 127.0.0.1 \
            --port "$server_port" \
            --no-ui \
...
run_arm() {
    ...
    arm_router_preset=$output_directory/$arm_label.router-preset.ini
    arm_router_staging=$output_directory/$arm_label.router-staging
    arm_router_log=$output_directory/$arm_label.router-preset.log
    arm_timeout_s=${QWEN_PROJECTOR_ARM_TIMEOUT_S:-$((300 + arm_depth / 2))}
    # A standalone arm names four retained artifacts; a router arm names the
    # generated section beside them, because the section rather than the argv
    # states the tuple the arm served at.
    arm_mode_artifacts=''
    [ "$runtime_mode" = standalone ] || arm_mode_artifacts=$arm_router_preset
    ...
    if [ "$recorded_count" -eq 1 ]; then
        for retained_artifact in "$arm_log" "$arm_samples" "$arm_requests" \
            "$arm_identity" $arm_mode_artifacts; do
    ...
    if [ "$runtime_mode" = router-child ]; then
        build_router_preset "$arm_depth" "$arm_router_staging" \
            "$arm_router_preset" "$arm_router_log"
    fi
    start_kernel_capture "$arm_kernel"
    kernel_before=unavailable
    [ "$kernel_capture_method" = follow ] || kernel_before=$(kernel_line_count)
    printf 'arm_start_utc=%s label=%s depth=%s cache=%s/%s fa=%s projector=loaded runtime_mode=%s kernel_capture=%s\n' \
        "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$arm_label" "$arm_depth" \
        "$cache_type_k" "$cache_type_v" "$flash_attention" "$runtime_mode" \
    ...
    stop_server
    if [ "$runtime_mode" = router-child ] && ! wait_for_device_release; then
        arm_status=router-child-survived
        printf 'router_child_survived label=%s: a spawned child still holds the device\n' \
            "$arm_label" >&2
    fi
    stop_sampler
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\tloaded\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        ...
        "$hazard_class" "$arm_log" "$runtime_mode" >>"$summary"
    printf 'arm_stop_utc=%s label=%s runtime_mode=%s status=%s prompt_n=%s prefill_s=%s decode_tok_s=%s resets=%s faults=%s control=%s health=%s hazard_class=%s\n' \
        "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$arm_label" "$runtime_mode" \
        "$arm_status" \
        "$prompt_n" "$prefill_s" "$decode_rate" "$arm_resets" "$arm_faults" \
...
run_arm_requests() {
    ...
    QWEN_PROJECTOR_REQUEST_MODEL=$request_model_key \
        python3 - <<'PY'
import base64
import json
import mimetypes
...
# router_validate_model reads the routing key from the body's top-level "model"
# and answers "model name is missing from the request" without one, so a router
# arm names its section and a standalone arm sends the body it always sent.
request_model = os.environ.get("QWEN_PROJECTOR_REQUEST_MODEL", "")
...
def chat(text, max_tokens, ignore_eos):
    return post("/v1/chat/completions", routed({
...
def tokenize(text):
    return len(post("/tokenize", routed(
...
awk -F'\t' -v OFS='\t' -v model_id="$model_id" \
    ...
    -v measured_at="$measured_at" -v runtime_mode="$runtime_mode" \
    -v tuple_suffix="$tuple_id_suffix" '
    NR > 1 && $10 == "ok" && $18 == "ok" && $20 == "healthy" {
        print model_id "-d" $3 "-b" $4 "-ub" $5 "-proj" tuple_suffix, model_id,
            runtime_mode,
...
if [ "$device_corrupt" -ne 0 ]; then
    printf 'depth_projector=halted model_id=%s runtime_mode=%s output_directory=%s emitted_rows=%s\n' \
        "$model_id" "$runtime_mode" "$output_directory" "$emitted_rows"
    cat "$summary"
    exit 1
fi

printf 'depth_projector=completed model_id=%s runtime_mode=%s output_directory=%s emitted_rows=%s\n' \
    "$model_id" "$runtime_mode" "$output_directory" "$emitted_rows"
cat "$summary"
