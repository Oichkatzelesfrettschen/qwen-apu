...
# `required` to expected_projector_state `loaded`, and probe-depth-wedge.sh
# drives llama-bench, which takes no --mmproj and allocates no projector
# buffers. Every arm that harness records therefore reads
...
run_arm() {
    ...
    # whole-device accounting: the peak sums every process's allocation, so it
    # names how full the device got rather than what this arm privately holds.
...
# than into remote/validated-tuples.tsv: a validated row requires its evidence
