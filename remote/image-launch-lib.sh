...
require_image_parameters() {
    ...
    # The parameter file states the geometry and the ceilings the service
    # enforces, so it is a generated byte of the appliance and lives under the
    # declared root like every other. The comparison is on the resolved path,
    # so a symlink under the root pointing outside it is the same refusal.
    # QWEN_IMAGE_PARAMETERS_EXTERNAL=1 admits a path outside it, which two
    # callers need: a harness writing its own parameter set into its own
    # output directory, as admit-image-router.sh does, and an operator serving
    # a bundle assembled before the parameter file was declared, whose MCP
    # configuration names the predecessor path and whose digest
    # verify-deployment-bundle.sh already binds. The migration is to
    # regenerate the preset against the declared path.
    image_parameters_resolved=$(readlink -f -- "$image_profiles_json")
    case $image_parameters_resolved in
        "$qwen_home"/*) ;;
        *)
            if [ "${QWEN_IMAGE_PARAMETERS_EXTERNAL:-0}" != 1 ]; then
                printf 'QWEN_IMAGE_PROFILES_JSON resolves to %s, outside the runtime root %s\n' \
                    "$image_parameters_resolved" "$qwen_home" >&2
                printf 'the declared parameter file is %s: regenerate the preset against it, or export QWEN_IMAGE_PARAMETERS_EXTERNAL=1 to serve a bundle that names the predecessor path\n' \
                    "$qwen_home_image_parameters" >&2
                return 1
            fi
            printf 'image_parameters_external=%s\n' "$image_parameters_resolved"
            ;;
    esac
