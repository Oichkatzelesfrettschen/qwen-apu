...
asked for (`sampler_nice`, `sampler_affinity`); `run-served-binary-ab.sh` and
`run-prefill-ladder.sh` pass both flags from the same `sidecar_nice` and
`sidecar_cpu` values their own sampler invocation carries, so the check reads
the priority and affinity each campaign actually configured rather than
`not_run`. `channel_cadence` and `cadence_values` hold a `# sample_rates:`
header to completeness -- `gpu_busy_percent_period_ns` and
