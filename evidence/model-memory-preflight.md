...
The first Qwen3.5-4B preflight will use the actual file size and a working-set
estimate that includes weights, KV, recurrent state, graph buffers, and staging
margin. The estimate must be replaced by measured peak allocation after each
context-depth run. No 9B or 27B model is opened when the live gate rejects it.
