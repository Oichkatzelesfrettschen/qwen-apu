...
characters in 1 s, and the 261-row audit names `category=qwen-open` while holding no
...
redaction and ahead of the hostname and home-prefix substitution, so what was removed
stays checkable against a re-run without being republished.

Result IDs are left as the harness recorded them. A Result ID is the signed claim over
one canonical URL that `web_fetch_exa` redeems, it reaches the model's own transcript by
design, and `evidence/web-admission-router-tools.md` and the retained `web-open` run
